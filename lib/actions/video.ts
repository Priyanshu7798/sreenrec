...
const waitUntilProcessed = async (videoId: string, maxRetries = 10, delay = 3000): Promise<boolean> => {
  let attempts = 0;
  ...
  while (attempts < maxRetries) {
    ...
    const status = processingInfo.status;
    if (status === 4) return true; // 4 = Ready
...
export const saveVideoDetails = withErrorHandling(
  async (videoDetails: VideoDetails) => {
    ...
    // ✅ Wait until video is processed (status === 4)
    const isReady = await waitUntilProcessed(videoDetails.videoId);
    if (!isReady) {
      throw new Error("Video processing timed out. Try again in a few moments.");
    }
